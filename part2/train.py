...
def _collect_async_sql(pending, cfg, ckpt_dir, model, best_val, best_metrics,
                       epochs_since_improvement, optimizer, mlflow_run_id):
    ...
    """
    future, p_epoch, p_eval_loss, p_tr_loss, p_avg_grad_norm, p_lr, \
    ...
    print(f"Epoch {p_epoch}: dev loss = {p_eval_loss:.4f}, F1 = {record_f1:.4f}, "
          f"EM = {record_em:.4f}, SQL_EM = {sql_em:.4f}, err = {error_rate*100:.1f}%")

    log_epoch_metrics({
        "epoch": p_epoch,
        "train_loss": p_tr_loss,
        "dev_loss": p_eval_loss,
        "record_f1": record_f1,
        "record_em": record_em,
        "sql_em": sql_em,
...
def train(cfg, model, train_loader, dev_loader, optimizer, scheduler, run_dir,
    ...
    _pending = None  # background SQL future + metadata
    try:
      for epoch in range(start_epoch, cfg.num_epochs):
        # ── Collect previous epoch's async SQL results ──
        if _pending is not None:
            best_val, best_metrics, epochs_since_improvement, should_stop = _collect_async_sql(
                _pending, cfg, ckpt_dir, model, best_val, best_metrics,
                epochs_since_improvement, optimizer, mlflow_run_id,
            )
            _pending = None
            if should_stop:
        ...
        if should_eval:
            eval_loss, all_preds = eval_epoch_gpu(cfg, model, dev_loader, device)
            ...
            if is_last_epoch:
                ...
                log_epoch_metrics({
                    "epoch": epoch, "train_loss": tr_loss, "dev_loss": eval_loss,
                    "record_f1": record_f1, "record_em": record_em,
                    "sql_em": sql_em, "error_rate": error_rate,
                ...
                print(f"Epoch {epoch}: train loss = {tr_loss:.4f}")
                print(f"Epoch {epoch}: dev loss = {eval_loss:.4f}, SQL metrics pending (async)...")
                future = _sql_pool.submit(
                    eval_epoch_sql, all_preds, cfg, gt_sql_path, model_sql_path,
                    gt_record_path, model_record_path, _pred_cache,
                )
                _pending = (future, epoch, eval_loss, tr_loss, avg_grad_norm,
        ...
        if cfg.max_wall_clock_hours and wall_clock >= cfg.max_wall_clock_hours * 3600:
            # Collect any pending SQL before stopping
            if _pending is not None:
        ...
        _interrupted = True
        # Collect pending SQL if possible
        if _pending is not None:
            try:
                best_val, best_metrics, epochs_since_improvement, _ = _collect_async_sql(
                    _pending, cfg, ckpt_dir, model, best_val, best_metrics,
...
def eval_epoch_gpu(cfg, model, dev_loader, device):
    """Phase A (GPU): compute dev loss + beam-search predictions."""
    model.eval()
    total_loss = 0
    total_tokens = 0
    criterion = LOSS_FNS[cfg.loss_fn]()
